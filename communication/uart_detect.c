...
// ── Common UART baud rates to probe ──────────────────────────────────────────
...
typedef struct {
    const char* name;
    const char* probe;       // string to send
    const char* response;    // expected substring in response
    uint32_t    baud;
} uart_device_t;
...
// ── Determine which UART instance a pin belongs to ───────────────────────────
static uart_inst_t* pin_to_uart(uint8_t rx_pin) {
    // UART0 RX: GP1, GP13, GP17, GP29
    // UART1 RX: GP5, GP9, GP21, GP25
    switch (rx_pin) {
        case 1: case 13: case 17: case 29: return uart0;
        case 5: case 9:  case 21: case 25: return uart1;
        default: return NULL;
    }
}

// ── Measure pulse widths on pin to estimate baud ──────────────────────────────
static uint32_t estimate_baud_from_pulses(uint8_t pin, uint32_t timeout_ms) {
    ...
    // Measure up to 64 pulse widths
    uint32_t widths[64];
    int      wcount = 0;
    int      last   = gpio_get(pin);
    uint64_t t_last = time_us_64();

    uint32_t measure_end = to_ms_since_boot(get_absolute_time()) + 50; // 50ms window
...
// ── Try to communicate with a device at a given baud ─────────────────────────
static const char* probe_device(uart_inst_t* port, uint8_t tx_pin, uint8_t rx_pin,
                                 uint32_t baud) {
    uart_init(port, baud);
    ...
    for (int d = 0; d < DEVICE_COUNT && !matched; d++) {
        ...
        if (!dev->response) continue;  // skip passive listeners here
        ...
        // Collect response for up to 800ms
        int pos = 0;
    ...
    // Check for passive NMEA ($GP) without probing
    if (!matched) {
        int pos = 0;
        uint32_t t0 = to_ms_since_boot(get_absolute_time());
...
// ── LA-assisted protocol identification ──────────────────────────────────────
void la_detect_protocol(uint8_t pin, int samples, int us_per_sample) {
    ...
    // UART: min pulse ~= 1 bit time; should be a clean divisor
    if (min_us >= 5) {
    ...
    // I2C: bursts with SCL clock ~100kHz (10us) or 400kHz (2.5us)
